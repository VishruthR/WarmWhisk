...
class PrometheusRecorderTests extends KafkaSpecBase with BeforeAndAfterEach with PrometheusMetricNames {
  ...
  it should "push user events to kamon" in {
    ...
    histogramCount(totalTimeMetric, namespaceDemo, actionWithCustomPackage) shouldBe 1
    histogramSum(totalTimeMetric, namespaceDemo, actionWithCustomPackage) shouldBe (1.284 +- 0.01)
